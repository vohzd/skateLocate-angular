...
(function(process) {
  (function(angular) {
    ...
    angular.module('leaflet-directive').service('leafletData', ["$log", "$q", "leafletHelpers", function($log, $q, leafletHelpers) {
      ...
      var _privateItems = ['map', 'tiles', 'layers', 'paths', 'markers', 'geoJSON', 'UTFGrid', 'decorations', 'directiveControls'];
    ...
    angular.module('leaflet-directive').factory('leafletLayerHelpers', ["$rootScope", "$log", "$q", "leafletHelpers", "leafletIterators", function($rootScope, $log, $q, leafletHelpers, leafletIterators) {
      ...
      var layerTypes = {
        ...
        geoJSONAwesomeMarker: {
          mustHaveUrl: false,
          createLayer: function(params) {
            return new L.geoJson(params.data, {pointToLayer: function(feature, latlng) {
        ...
        custom: {createLayer: function(params) {
    ...
    angular.module('leaflet-directive').factory('leafletPathsHelpers', ["$rootScope", "$log", "leafletHelpers", function($rootScope, $log, leafletHelpers) {
      ...
      var availableOptions = ['stroke', 'weight', 'color', 'opacity', 'fill', 'fillColor', 'fillOpacity', 'dashArray', 'lineCap', 'lineJoin', 'clickable', 'pointerEvents', 'className', 'smoothFactor', 'noClip'];
    ...
    angular.module('leaflet-directive').factory('nominatimService', ["$q", "$http", "leafletHelpers", "leafletMapDefaults", function($q, $http, leafletHelpers, leafletMapDefaults) {
      var isDefined = leafletHelpers.isDefined;
      return {query: function(address, mapId) {
          var defaults = leafletMapDefaults.getDefaults(mapId);
                ...
                for (key in newOverlayLayers) {
                  ...
                  if (isDefined(layer.group)) {
                    if (!isDefined(scope.groupProperties[layer.group])) {
                      scope.groupProperties[layer.group] = {visible: false};
                    }
      ...
      var _addMarkers = function(mapId, markersToRender, oldModels, map, layers, leafletMarkers, leafletScope, watchOptions, maybeLayerName, skips) {
        for (var newName in markersToRender) {
          ...
          if (!isDefined(maybeLMarker)) {
            ...
            var layerName = (model ? model.layer : undefined) || maybeLayerName;
            if (!isDefined(marker)) {
            ...
            if (isDefined(model) && (isDefined(model.layer) || isDefined(maybeLayerName))) {
              var pass = _maybeAddMarkerToLayer(layerName, layers, model, marker, watchOptions.individual.doWatch, map);
              if (!pass)
                continue;
        ...
        link: function(scope, element, attrs, controller) {
          ...
          mapController.getMap().then(function(map) {
            ...
            if (isDefined(attrs.watchMarkers))
              watchOptions.doWatch = watchOptions.individual.doWatch = (!isDefined(attrs.watchMarkers) || Helpers.isTruthy(attrs.watchMarkers));
        ...
        link: function(scope, element, attrs, controller) {
          ...
          controller.getMap().then(function(map) {
            ...
            leafletScope.$watch('tiles', function(tiles, oldtiles) {
              ...
              if (isDefined(tiles.url) && isDefined(tiles.options) && (tiles.type !== oldtiles.type || !angular.equals(tiles.options, tileLayerOptions))) {
                map.removeLayer(tileLayerObj);
        ...
        if (!isDefined(leafletScope.eventBroadcast)) {
          ...
          if (!isDefined(leafletScope.eventBroadcast[_this.lObjectType])) {
            ...
            if (isDefined(leafletScope.eventBroadcast[this.lObjectType].logic)) {
              if (leafletScope.eventBroadcast[_this.lObjectType].logic !== 'emit' && leafletScope.eventBroadcast[_this.lObjectType].logic !== 'broadcast')
                $log.warn(errorHeader + 'Available event propagation logic are: \'emit\' or \'broadcast\'.');
            }
            var eventsEnable = false;
            var eventsDisable = false;
            if (isDefined(leafletScope.eventBroadcast[_this.lObjectType].enable) && isArray(leafletScope.eventBroadcast[_this.lObjectType].enable))
              eventsEnable = true;
            if (isDefined(leafletScope.eventBroadcast[_this.lObjectType].disable) && isArray(leafletScope.eventBroadcast[_this.lObjectType].disable))
              eventsDisable = true;
